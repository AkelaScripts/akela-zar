...
function DrawText3Dz(x,y,z, text)
    ...
    if onScreen then
        ...
        DrawRect(_x,_y+0.0135, 0.010+ factor, 0.03, 0, 0, 0, 50)
    end
  end
local show = {}
function showZar() 
	...
	ExecuteCommand('ooc Atılan zar: '..zar)
	while show == true do 
		Wait(1)
			local text = '~w~Zarın üzerinde ~b~'..zar..'~w~ yazdığı görülebilir.'
			local cord = GetEntityCoords(ped)
			DrawText3Dz(cord.x, cord.y, cord.z, text)
	end
end
